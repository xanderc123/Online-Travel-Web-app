    ...
    <el-row style="background-color: #FFFFFF;padding: 5px 30px;border-radius: 5px;">
        <el-row style="padding: 10px;margin: 0 5px;">
            <el-row>
                ...
                <el-input size="small" style="width: 188px;margin-left: 5px;margin-right: 6px;"
                    v-model="attractionTicketOrderQueryDto.userId" placeholder="User ID" clearable
        ...
        <el-row style="margin: 0 15px;border-top: 1px solid rgb(245,245,245);">
            ...
            <el-table :data="tableData" style="width: 100%">
                ...
                <el-table-column prop="userName" width="120" label="Username"></el-table-column>
                ...
                <el-table-column prop="useStatus" width="98" label="Payment Status">
                    <template slot-scope="scope">
                        ...
                        <el-tooltip v-if="!scope.row.payStatus" class="item" effect="dark"
                            content="Not paid, cannot re-order" placement="bottom-end">
        ...
        <el-row>
            <LineChart height="480px" tag="Ticket Sales ($)" @on-selected="selected" :values="values" :date="dates" />
        </el-row>
    </el-row>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            values: [],
            dates: [],
            statusList: [{ value: null, label: 'All' }, { value: 0, label: 'Not Paid' }, { value: 1, label: 'Paid' }],
        };
    },
    created() {
        this.fetchFreshData();
    ...
    methods: {
        // 请求后端的金额成交数据
        selected(time) {
            this.$axios.get(`/attractionTicketOrder/daysQuery/${time}`).then(response => {
                const { data } = response;
                if (data.code === 200) {
        ...
        async fetchFreshData() {
            try {
                ...
                const response = await this.$axios.post('/attractionTicketOrder/queryAttractionTicketOrder', params);
                const { data } = response;
